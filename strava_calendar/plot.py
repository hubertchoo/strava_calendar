...
class Run:
    ...
    def get_scale(self, xlims=(0.1, 0.9), ylims=(0.1, 0.7)):
...
class Plotter:
    def __init__(self, data, zip_path, show_mileage, plot_period):
        self.years = {}
        self.months = {}
        self.days = {}
        self.zip_path = zip_path
        self.show_mileage = show_mileage
        self.plot_period = plot_period
        for activity in data["activities"]:
            self.add_activity(activity)
        self.total_distance = {} #dict to store total distance run each day
        self.load_total_distance()


    def load_total_distance(self):
        #added to get the total distance run in that day
        run_zip = ZipFile(self.zip_path)
        with run_zip.open("activities.csv", 'r') as activity_data:
            csv_reader = csv.reader(TextIOWrapper(activity_data, 'utf-8'))
            line = next(csv_reader)
            for line in csv_reader:
                date = line[1].split(",")
                date = date[0]
                date = datetime.datetime.strptime(date, "%d %b %Y").date()
                distance = float(line[6])
                if date not in self.total_distance.keys():
                    self.total_distance[date] = 0
                self.total_distance[date] += distance
    ...
    def plot_month(self, *, year, month, fig, ax, extra_offset=np.array([0, 0])):
        ...
        while date.year == year and date.month == month:
            ...
            if date in month_data:
                if self.show_mileage and self.plot_period == 'year':
                    ax.text(day - extra_offset[0] + 0.6, week + 0.05 - extra_offset[1] + 0.8, '{0:.2f}'.format(self.total_distance[date]), alpha=0.5, fontsize=6)
                elif self.show_mileage and self.plot_period == 'month':
                    ax.text(day - extra_offset[0] + 0.65, week + 0.05 - extra_offset[1] + 0.8, '{0:.2f} km'.format(self.total_distance[date]), alpha=0.5, fontsize=6)
