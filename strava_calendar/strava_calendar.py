...
show_mileage = True #set True to show distance ran per day
...
def plot_year(
    *,
    ...
    label : str or NoneLabel in the top left corner of the plots. Defaults to the year. Use ""
    ...
    plotter = Plotter(data, zip_path, show_mileage, plot_period="year")
...
def plot_month(
    *,
    ...
    data = get_data(zip_path, sport, datetime.datetime(year, 1, 1), datetime.datetime(year+1, 1, 1))
    plotter = Plotter(data, zip_path, show_mileage, plot_period="month")
    fig, ax = plt.subplots(figsize=(9, 6))
    fig, ax, offset = plotter.plot_month(year=year, month=month, fig=fig, ax=ax)
    ax.text(0, offset + 5.2, label, fontdict={'fontsize': 32, 'fontweight': 'heavy'}, alpha=0.5)
    plt.show()
    return fig, ax
